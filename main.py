...
title.config(font=("TimesNewRoman", 40))
...
def aboutPressed():
  ...
  about = Label(about_frame, text = "About the program", font = ("TimesNewRoman", 25))
  ...
  lab5 = Label(about_frame, borderwidth=0, highlightthickness=0)
  img5 = PhotoImage(file = dir + "/Images/discord.png",  width=150, height=150)
  lab5['image'] = img5
  lab5.grid(row=13, column=5)

  about.grid(row=5, column=5)
  info.grid(row=8, column=5)
  customer_reviews.grid(row = 9, column = 5)
  
  back_button = Button(about_frame, text = "Back", bg = "#ff6666", activebackground = "#f54040", command = back)
...
def retrieveInfo():
  ...
  if(tasks.validate_time(data_start) and tasks.validate_time(data_end)):
    if(tasks.validate_start_end(data_start, data_end)):
...
def schedulePressed():
  ...
  submit_task_button = Button(schedule_frame, text = "Submit", height=1, width=10, bg="lightGreen", activebackground = "#2bf070", command = retrieveInfo)
  submit_task_button.config(font=("TimesNewRoman", 20))
  submit_task_button.grid(row = 2 , column = 1)
  
  back_button = Button(current_frame, text = "Back", bg = "#ff6666", activebackground = "#f54040", command = back)
...
def creditsPressed():
  ...
  credits = Label(credit_frame, text = "This application was created by Jason W, Nathan H, and Kelvin F. \n This program was created for LyonHacks II.\n Thank you to William Lyon Mackenzie CI, \nRonald You, Kenneth Ruan, Justin Lu, \nBen Zeng, Jeremy Liang, Hazel Guo, Jimmy Liu, Vlad Surdu \nand ECOO for giving us the opportunity to participate \nin an excellent hackathon", font = ("TimesNewRoman", 25))
  ...
  back_button = Button(credit_frame, text = "Back", bg = "#ff6666", activebackground = "#f54040", command = back)
...
def motivationPressed():
  ...
  motivation.config(font=("TimesNewRoman", 15))
  ...
  back_button = Button(motivation_frame, text = "Back", bg = "#ff6666", activebackground = "#f54040", command = back)
  back_button.grid(row = 8, column = 5)
...
def viewPressed():
  ...
  if len(task_list) == 0:
    no_tasks = Label(view_frame, text="There are no scheduled tasks.", height=2, width = 30)
    no_tasks.config(font=("TimesNewRoman", 20))
    no_tasks.grid(row = 8, column = 1)
    schedule_button = Button(view_frame, text="Schedule new tasks", height=2, width = 20, bg="lightGreen", activebackground = "#2bf070", command=viewToSchedule)
    schedule_button.config(font=("TimesNewRoman", 15))
    ...
    for task in range(len(task_list)):
      for property in range(3):
        ...
        if(property == 0):
          ...
          cell.config(state = 'readonly', font=("TimesNewRoman", 20))
        elif(property == 1):
      ...
      check.config(font=("TimesNewRoman", 15))
      check.grid(row = task+1, column = 4)
    
    remove_tasks_button = Button(view_frame, text="Remove completed tasks", bg="lightGreen", activebackground = "#2bf070", command=remove_tasks)
    remove_tasks_button.config(font=("TimesNewRoman", 20))
    remove_tasks_button.grid(row = 8, column = 1)
  
  back_button = Button(view_frame, text = "Back", bg = "#ff6666", activebackground = "#f54040", command = back)
  back_button.config(font=("TimesNewRoman", 10))
  back_button.grid(row = 10, column = 1)

#Buttons
aboutButton = Button(welcome_widget_frame, text = "About", bg="Yellow", activebackground = "#f7dd14", height = 2, width = 20, command=aboutPressed)

scheduleButton = Button(welcome_widget_frame, text = "Schedule New Tasks", bg="lightGreen", activebackground = "#2bf070", height = 2,width = 20, command=schedulePressed)

tasksButton = Button(welcome_widget_frame, text = "View Tasks", bg = "lightBlue", activebackground = "skyBlue", height = 4, width = 20, command=viewPressed)

creditsButton = Button(welcome_widget_frame, text = "Credits", bg = "#ffbe63", activebackground = "orange", height = 2, width = 20, command=creditsPressed)

motivationButton = Button(welcome_widget_frame, text = "Motivational Quote", bg = "#ff6666", activebackground = "#f54040", height = 2, width = 20, command=motivationPressed)
